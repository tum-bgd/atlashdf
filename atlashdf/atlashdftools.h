...
void flush_array(group_or_file f, std::string dataset_name, array_type &a,
  ...
  size[0] += a.size();
  ds.resize(size);
  ds.select({start_row, 0}, {a.size(), num_columns})
      .write(a); // note that a is not empty here
}
...
struct ChunkedOSMImmediateWriter {
  ...
  void flush_nodes() {
    flush_array(g(), "nodes_coords", coord_cache,
                2); // last param is #cols in hdf5
    coord_cache.clear();
    flush_array(g(), "nodes_attr", node_attribs,
                1); // last param is #cols in hdf5
    node_attribs.clear();
